...
btn_enter = Button(root, text="enter")  #세로로 2개가 1개로 합쳐진 타입

btn_1.grid(row=4, column=0)
btn_2.grid(row=4, column=1)
btn_3.grid(row=4, column=2)
btn_enter.grid(row=4, column=3, rowspan=2)  #rowspan=2  => row2개를 합치겠다! 현재 위치로부터 아래쪽으로 n줄을 더함

#0 시작 줄  => 2개만 처리하면 된다.
btn_0 = Button(root, text="0")             #가로로 2개가 합쳐짐. 
btn_point = Button(root, text=".")

btn_0.grid(row=5, column=0, columnspan=2)   #columnspan ==> 현재 위치로부터 오른쪽으로 n칸 더함
